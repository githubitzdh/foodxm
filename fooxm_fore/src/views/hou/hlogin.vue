<template>
 ...
 请点击此处会员入口>>><el-link type="info" href="/login"><span style=" font-size: 10px;">用户登录</span> </el-link> 
      </el-form>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    loginFn(formName) {
      this.$refs[formName].validate(async valid => {
        ...
        if (valid) {
          //发送axios
          let { data: baseresult } = await hlogin(this.user);
          //处理
...
.login .login-box {
  ...
  background: url("~@/assets/13.jpg") no-repeat;
  /* 调整图片大小 */
