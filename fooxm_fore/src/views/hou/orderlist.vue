  ...
  <div>
    ...
    <el-card class="box-card">
      ...
      <el-table ref="singleTable" :data="orderForm.list" highlight-current-row style="width: 100%">
        ...
        <el-table-column label="下单用户" width="100">
          <template slot-scope="scope">{{ scope.row.userlist.username }}</template>
        </el-table-column>
        ...
        <el-table-column label="状态" width="100">
          <template slot-scope="scope">
            <el-tag v-if="scope.row.status==1" type="info">
              <div>未处理</div>
              <el-link type="primary" @click="upstatueFn(scope.row.oid)">点击处理</el-link>
            </el-tag>
            <el-tag v-if="scope.row.status==2" type="success">已处理</el-tag>
          </template>
        </el-table-column>

        <el-table-column label="下单" width="210">
          <template slot-scope="scope">
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button
              ...
              circle
              @click="openLookorder(scope.row.oid)"
            ></el-button>
            <el-button type="primary" icon="el-icon-edit" circle @click="opened(scope.row.oid)"></el-button>
            <el-button type="danger" icon="el-icon-delete" circle @click="delFn(scope.row.oid)"></el-button>
          </template>
        </el-table-column>
      </el-table>
      <!-- 表单end -->
      ...
      <!-- 修改会员start -->
      <el-dialog title="修改订单" :visible.sync="dialogedble" width="30%" :before-close="handleClose">
        <el-form ref="edForm" :model="edForm" label-width="80px" status-icon>
          <el-form-item label="总价">
            <el-input v-model="edForm.total"></el-input>
          </el-form-item>

          <el-form-item label="收货地址">
            <el-input v-model="edForm.address"></el-input>
          </el-form-item>
        ...
        <span slot="footer" class="dialog-footer">
          ...
          <el-button type="primary" @click="edFn">确 定</el-button>
        </span>
      </el-dialog>
      <!-- 修改会员end -->

      <!-- 查看订单start -->
      <el-dialog title="查看订单" :visible.sync="dialogorble" width="30%" :before-close="handleClose">
        ...
        <div>下单日期: {{ordertimeinfo}}</div>
        <div>收货地址: {{addressinfo}}</div>

        <div>收货人: {{nameinfo}}</div>
        <div>联系电话: {{telephoneinfo}}</div>
...
export default {
  ...
  methods: {
    ...
    async findall() {
      let { data: a } = await orderpage(this.params);
    ...
    async upstatueFn(id) {
      let { data: e } = await upstatue(id);
      this.$message.success(e.message);
      this.findall();
    },
    async opened(id) {
      this.dialogedble = true;
      let { data: e } = await searchbyorderid(id);
      this.edForm = e.data;
    },
    async edFn() {
      let { data: e } = await uporder(this.edForm);
      if (e.code == 1) {
        this.$message.success(e.message);
